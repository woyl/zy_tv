...
public class GridVideoViewContainer extends RecyclerView {
    ...
    private boolean initAdapter(int localUid, HashMap<Integer, TextureView> uids, boolean isFullscreen) {
        if (mGridVideoViewContainerAdapter == null) {
            mGridVideoViewContainerAdapter = new GridVideoViewContainerAdapter(getContext(), localUid, uids, mEventListener, isFullscreen);
        ...
        public void initViewContainer(Context context, int localUid, HashMap<Integer, TextureView> uids, boolean isFullscreen) {
        boolean newCreated = initAdapter(localUid, uids, isFullscreen);

        if (!newCreated) {
            mGridVideoViewContainerAdapter.setLocalUid(localUid);
            mGridVideoViewContainerAdapter.init(uids, localUid, true, isFullscreen);
        }
